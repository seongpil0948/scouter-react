// frontend/app/(routes)/services/[name]/page.tsx
'use client';
...
import { useFilterStore, ServiceMetric, TraceItem, LogItem } from '@/lib/store/telemetryStore';
    ...
    <>
      ...
      <div className="mt-4">
        <div className="flex justify-between items-center mb-4">
          <Button className="flex items-center gap-1" variant="ghost" onClick={handleBack}>
            <ArrowLeft size={16} />
            <span>서비스 목록으로</span>
          </Button>

          <Button className="flex items-center gap-1" variant="ghost" onClick={handleRefresh}>
            <RefreshCw size={16} />
            <span>새로고침</span>
          </Button>
        </div>

        <Tabs className="w-full mb-4" selectedKey={activeTab} onSelectionChange={setActiveTab}>
        ...
        {activeTab === 'metrics' ? (
          ...
          <TraceList isLoading={isLoadingTraces} traces={traces} onSelectTrace={handleViewTrace} />
        ) : (
          <LogList isLoading={isLoadingLogs} logs={logs} onSelectLog={handleViewLog} />
        )}
      </div>
    </>
  );
