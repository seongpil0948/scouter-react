...
const LogVisualization: React.FC<LogVisualizationProps> = ({
  ...
  }, [chartTitle, theme, colors, symbolSizes]);
  ...
  const processLogData = useCallback(
    (newLogs: LogItem[]) => {
      ...
      if (hasNewData) {
        ...
        try {
          // 시리즈 데이터 준비
          const seriesData = Object.keys(chartDataRef.current).map(
            (severity, index) => {
              console.log(
                `Series ${severity} has ${chartDataRef.current[severity].length} points`,
              ); // 디버깅 로그 추가
              return {
