...
export function useTraceVisualization(options: {
  ...
  const cachedDataRef = useRef<{[key: string]: {timestamp: number, data: TraceItem[]}}>({}); 
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const error = err instanceof Error ? err : new Error('알 수 없는 오류 발생');
      setError(error);
      return [];
    } finally {
      setIsLoading(false);
  ...
  // 시간 범위 업데이트 함수
  const updateTimeRange = useCallback((newTimeRange: Partial<typeof timeRange>) => {
    setTimeRange(prev => ({
      ...prev,
      ...newTimeRange,
    }));
  ...
  // 시간 범위 프리셋 함수
  const setTimeRangePreset = useCallback((preset: 'hour' | 'day' | 'week' | 'month') => {
    const now = Date.now();
    let startTime;
