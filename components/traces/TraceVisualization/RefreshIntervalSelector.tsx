...
import { useChartStore, RefreshIntervalOption } from "@/lib/store/chartStore";
...
const RefreshIntervalSelector: React.FC<RefreshIntervalSelectorProps> = ({
  ...
  const handleIntervalChange = (key: any) => {
    ...
    <div className={`flex items-center gap-2 ${className}`}>
      <div className="flex items-center gap-2">
        <Tooltip content={autoRefreshEnabled ? "자동 새로고침 켜짐" : "자동 새로고침 꺼짐"}>
          <RefreshCw 
            className={`${autoRefreshEnabled ? "text-blue-500" : "text-gray-400"}`}
            size={18} 
          />
        </Tooltip>
