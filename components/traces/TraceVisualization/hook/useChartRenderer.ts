...
export function useChartRenderer({
  ...
  const isRealtime = useMemo(
    () => mergedConfig.autoUpdate === true,
    [mergedConfig.autoUpdate]
  );
  ...
  const handleBrushSelected = useCallback(
    (params: BrushParams) => {
      ...
      // 각 배치 처리
      params.batch.forEach((batchItem: BatchItem) => {
        // Type batchItem
        ...
        // 각 시리즈 데이터 처리
        batchItem.selected.forEach((selection: SelectionItem) => {
          // Type selection
    ...
    try {
      ...
      if (onBrushSelected) {
        ...
        instance.on("brushselected", (params: any) =>
          handleBrushSelected(params as BrushParams)
        );
      }
      ...
      if (onDataPointClick) {
        const clickHandler = (params: any) => {
          ...
          if (
            ...
            // params.value는 [timestamp, latency] 형태
            if (
              params.value &&
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (onBrushSelected) {
        ...
        chartInstanceRef.current.on("brushselected", (params: any) =>
          handleBrushSelected(params as BrushParams)
        );
      }
      ...
      const hasHighLatencyData =
        data.highLatencyData && data.highLatencyData.length > 0;

      if (hasTimeSeriesData || hasHighLatencyData) {
        // 모든 데이터 포인트 통합
        const allPoints = [
          ...(legendState.normal && data.timeSeriesData
            ? data.timeSeriesData
            : []),
          ...(legendState.highLatency && data.highLatencyData
            ? data.highLatencyData
            : []),
        ];
        ...
        if (allPoints.length > 0) {
          ...
          const xAxisOption =
            prevOption?.xAxis as echarts.EChartsOption["xAxis"];
          const prevXAxis = Array.isArray(xAxisOption)
            ? xAxisOption[0]
            : (xAxisOption ?? {});
          ...
          if (
            isRealtime &&
            prevMinTime !== undefined &&
            prevMaxTime !== undefined
          ) {
            // 윈도우 크기 유지 (시간 이동)
