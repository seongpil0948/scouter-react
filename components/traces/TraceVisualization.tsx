...
const TraceVisualization: React.FC<TraceVisualizationProps> = ({ traceData, onDataPointClick, config = {}, height, title }) => {
  const mergedConfig = {
    ...DEFAULT_CONFIG,
    ...config,
  ...
  const processTraceData = useCallback(
    (newTraces: TraceItem[]) => {
      ...
      newTraces.forEach((trace) => {
        ...
        const timestamp = typeof trace.startTime === 'string' ? parseInt(trace.startTime, 10) : trace.startTime;
      ...
      if (hasNewData) {
        ...
        try {
          ...
          chartInstanceRef.current?.setOption({
            series: [{ data: updatedTimeSeriesData }, { data: updatedHighLatencyData }],
          });
