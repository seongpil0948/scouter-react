...
const SqlAttributesViewer: React.FC<SqlAttributesViewerProps> = ({ attributes, className = '' }) => {
  // SQL 관련 속성 추출
  ...
  const handleCopySql = () => {
    ...
    navigator.clipboard.writeText(String(sqlQuery))
      .then(() => {
        addToast({
          title: 'SQL 복사 완료',
  ...
  return (
    <div className={`${className} bg-white dark:bg-gray-800 rounded-lg overflow-hidden`}>
      <div className="border-b dark:border-gray-700">
        ...
        {sqlQuery ? (
          <div className="p-4 overflow-auto max-h-96">
            <pre className="font-mono text-sm whitespace-pre-wrap bg-gray-50 dark:bg-gray-900 p-3 rounded-md border border-gray-200 dark:border-gray-700">
              {String(sqlQuery)}
            </pre>
          </div>
        ) : (
      ...
      {/* 기타 SQL 관련 속성 표시 */}
      <div className="p-4">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">SQL 관련 정보</h4>
